<template>
  <div class="p-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
      ...
      <GradientButton @click="goToAddPortfolio" class="px-3 py-2 rounded-md font-semibold flex items-center gap-2 cursor-pointer">
    ...
    <div v-if="filteredPortfolios.length > 0" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <div v-for="portfolio in filteredPortfolios" :key="portfolio.id" class="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow p-6">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h2 class="text-lg font-semibold">{{ portfolio.name }}</h2>
            <p class="text-sm text-gray-500">{{ portfolio.clientName }}</p>
            <span class="inline-block mt-2 px-2 py-1 rounded text-xs font-semibold" :class="statusClasses(portfolio.status)">
              {{ portfolio.status }}
            </span>
          </div>
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <p class="text-2xl font-bold text-purple-600">{{ formatCurrencyINR(portfolio.totalValue) }}</p>
              <p class="text-xs text-gray-500">Current Value</p>
            </div>
            <div>
              <p class="text-2xl font-bold" :class="portfolio.returnPercentage >= 0 ? 'text-green-600' : 'text-red-600'">
          ...
          <div class="text-sm text-gray-500">
            <p>Initial: {{ formatCurrencyINR(portfolio.initialInvestment) }}</p>
            <p>P&amp;L: {{ formatCurrencyINR(portfolio.currentReturn) }}</p>
          </div>

          <div class="pt-3">
            <button @click="goToPortfolio(portfolio.id)" class="w-full px-3 py-2 rounded-md border border-gray-300 shadow-sm transition-colors bg-white hover:bg-gradient-to-r hover:from-purple-500 hover:to-purple-600 hover:text-white cursor-pointer">
              View Details
            </button>
          </div>
        </div>
...
const filteredPortfolios = computed(() => {
  return portfolios.value.filter((p) => {
    ...
    return matchesSearch && String(p.status).toLowerCase() === activeTab.value;
  });
});

const formatCurrencyINR = (amt) =>
  new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', minimumFractionDigits: 0 }).format(Number(amt ?? 0));
